...
static bool rhs_of_expression=0;
static bool lhs_of_assignment=0;
static bool value_in_R2=0;
...
class CompoundStatement : public Node {
	...
	public:
		...
		void render_asm(std::ofstream& file, bool initialized=false, bool function=true) {
			...
			if(StatementListPtr != NULL && DeclarationListPtr == NULL && !reading) {
...
inline void Statement::render_asm(std::ofstream& file,bool elseif) {
			...
			if( IterationStatementPtr != NULL){
				IterationStatementPtr->render_asm(file);
			}
			if( CompoundStatementPtr != NULL){
				CompoundStatementPtr->render_asm(file);
			}
			if( ExpressionStatementPtr != NULL){
				ExpressionStatementPtr->render_asm(file);
			}
...
inline void PrimaryExpression::render_asm(std::ofstream& file)  {
			...
			if( IDENTIFIER != NULL ) {
				...
				//process offset here
				if(lhs_of_assignment)
				{
					file << std::endl << "\tsw\t $2, " << offset << "($sp)";
					value_in_R2=0;
					lhs_of_assignment=0;
				}
				if(rhs_of_expression)
				{
				...
				placeholder = *IDENTIFIER; //what's placeholder?
			}
